...
class UserAction extends AdminbaseAction{
	...
	protected $users_obj_es,$users_obj_en,$users_obj_pt,$users_obj_ru,$users_obj_ar,$users_obj_de,$users_obj_kr,$users_obj_tw,$users_obj_cmf;
	...
	function add_post(){
		if(IS_POST){
			...
			if($en && $es && $fr && $pt && $ru && $ar && $de && $kr && $tw && $cmf){
				if($this->users_obj->add()!==false && $this->users_obj_es->add()!==false && $this->users_obj_en->add()!==false && $this->users_obj_pt->add()!==false && $this->users_obj_ru->add()!==false && $this->users_obj_ar->add()!==false && $this->users_obj_de->add()!==false && $this->users_obj_kr->add()!==false && $users_obj_tw->add()!==false && $this->users_obj_cmf->add()!==false){
	...
	function edit_post(){
		if (IS_POST) {
			...
			if ($en && $ar && $es && $fr && $pt && $ru && $de && $kr && $tw && $cmf) {
				...
				if ($result!==false && $es!==false && $en!==false && $pt!==false && $ru!==false && $ar!==false && $de!==false && $kr!==false && $tw!==false && $cmf!==false) {
					$this->success("保存成功！");
				} else {
					$this->error("保存失败！");
				}
